...
void check_symbol(AST *node) {
    if (node->symbol->type){
        if (node->symbol->type == SYMBOL_FUNCTION){
            fprintf(stderr,"Semantic ERROR: function needs parentheses.\n");
            ++SemanticErrors;
        } else if (node->symbol->type == SYMBOL_VECTOR){
            fprintf(stderr,"Semantic ERROR: unindexed vector.\n");
            ++SemanticErrors;
        } else if (node->symbol->type == SYMBOL_LABEL){
            fprintf(stderr,"Semantic ERROR: label declared incorrectly.\n");
            ++SemanticErrors;
        }
    }
}
